...
######## For comparison, construct a PCA plot of the same data ########
pca <- prcomp(t(data.matrix), 
              #center=T, 
              scale=T)
pca.var <- pca$sdev^2   # compute variance of each PC
pca.var.per <- round(pca.var/sum(pca.var)*100, 1)   # compute percentage variances

pca.data <- data.frame(Object=rownames(pca$x), PC1=pca$x[,1], PC2=pca$x[,2])   # column named 'Object' is the ID column
